...
import * as dataFormat from 'pages/CRUD/Tarifas/table/TarifasDataFormatters';

import actions from 'actions/tarifas/tarifasListActions';
...
const TarifasTable = () => {
  ...
  const [filters, setFilters] = React.useState([
          ...
          {label: 'Codigo Identificador', title: 'taR_COD_IDENTIF_TARIFA', number: 'true'},
          {label: 'Valor da Tarifa', title: 'vtF_VALOR_TARIFA', number: 'true'},
  ...
  const count = useSelector((store) => store.tarifas.list.count);
  const modalOpen = useSelector((store) => store.tarifas.list.modalOpen);
  const rows = useSelector((store) => store.tarifas.list.rows);
  const idToDelete = useSelector((store) => store.tarifas.list.idToDelete);
  ...
  const columns = [
      ...
      { field: "vtF_DATA_FIM_VALIDADE",

        renderCell: (params) => dataFormat.dateTimeFormatter(params.value),

      headerName: "Data Validade Final"
      },

      { field: "vtF_VALOR_TARIFA",

        flex: 0.6,

      headerName: "Valor da Tarifa"
      },

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="tarifas" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>Tarifas</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/tarifas/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/tarifas/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Tarifas'
                : 'http://localhost:8080/api-docs/#/Tarifas'
            }
          >
            API documentation for tarifas
          </LinkMaterial>
        </div>
      </Widget>
