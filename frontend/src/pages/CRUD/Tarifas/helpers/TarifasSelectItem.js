...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/tarifas/autocomplete`, { params });
  return response.data;
}

const TarifasSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.tarifas.hasPermissionToCreate,
});

export default connect(select)(TarifasSelectItem);
