...
import { TarifasRepository } from '../repository/TarifasRepository';
import { tarifasInputSchema, tarifasSchema } from '../entity/Tarifas';

export const tag = 'Tarifas';

export default async (app: FastifyInstance) => {
    const schema = TypeBox.Type.Object({
        q: TypeBox.Type.Optional(TypeBox.Type.Partial(tarifasSchema, { description: 'Filter query', additionalProperties: false })),
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/tarifas', {
        // @ts-ignore
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'List tarifas',
        },
    }, async (req) => {
        // @ts-ignore
        const [items, count] = await getCustomRepository(TarifasRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase());
        return {
            rows: items,
            count,
    ...
    app.get<{
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'Find tarifas instance to link as relations',
            response: {
        ...
        const repo = getCustomRepository(TarifasRepository);
    ...
    const postPayload = TypeBox.Type.Object({
        data: tarifasInputSchema,
    });
    app.post<{
        Body: TypeBox.Static<typeof postPayload>,
        Reply: TypeBox.Static<typeof tarifasSchema>
    }>('/tarifas', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
        ...
        schema: {
            ...
            response: {
                ...
                404: TypeBox.Type.Object({
                    ...
                    message: TypeBox.Type.String(),
                }, { description: 'Tarifas not found' }),
            },
        }
    // @ts-ignore
    }, async (req, reply) => {
        const entity = await getCustomRepository(TarifasRepository).findOne({
            where: {
                id: req.params.id,
            },
        ...
        return entity ? entity : reply.notFound('Tarifas not found');
    });

    const putSchema = TypeBox.Type.Object({
        id: TypeBox.Type.String({ format: 'uuid' }),
        data: TypeBox.Type.Partial(tarifasInputSchema),
    });
    app.put<{
        Params: TypeBox.Static<typeof find>,
    ...
    app.delete<{
        ...
        preHandler: app.auth([app.verifyAdmin]),
        schema: {
            description: 'Delete tarifas',
            summary: 'Delete tarifas',
            params: find,
            tags: [tag],
            security: [{
                bearerAuth: [],
            ...
            response: {
                200: {
                    description: 'tarifas successfully deleted',
                    type: 'null',
                },
                404: TypeBox.Type.Object({
                    statusCode: TypeBox.Type.Number({ default: 404 }),
        ...
        const entity = await getCustomRepository(TarifasRepository).softDelete(req.params.id);
        if (!entity.affected) {
            reply.notFound('tarifas not found');
            return;
        }
